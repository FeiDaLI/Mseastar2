...
//     }
//     future<> close() {
//         assert(!_stopped && "gate::close() cannot be called more than once");
...
//         void account_start();
//         void account_stop();
//         std::optional<std::chrono::steady_clock::time_point> next_scheduling_point() const;
// };
...
//     std::optional<promise<>> _sched_promise;
...
//         return stack;
//     }
//     thread_context(thread_attributes attr, std::function<void ()> func):_attr(std::move(attr)),_func(std::move(func)) {
//         setup();
...
//     ///             called immediately.
//     template <typename Func>
//     thread(Func func): thread(thread_attributes(), std::move(func)){}
//     /// \brief Constructs a \c thread object that represents a thread of execution
//     /// \param attr Attributes describing the new thread.
//     /// \param func Callable object to execute in thread.  The callable is
//     ///             called immediately.
//     template <typename Func>
//     thread(thread_attributes attr, Func func):_context(std::make_unique<thread_context>(std::move(attr), func)){}
...
// std::optional<thread_clock::time_point>
