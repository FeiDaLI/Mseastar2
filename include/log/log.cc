...
const std::map<log_level, std::string> log_level_names = {
...
logger::really_do_log(log_level level, const char* fmt, stringer** s, size_t n) {
    ...
    static std::unordered_map<int, std::string> level_map = {
        { static_cast<int>(log_level::debug), log_level_to_string(log_level::debug) },
        { static_cast<int>(log_level::info), log_level_to_string(log_level::info) },
        { static_cast<int>(log_level::trace), log_level_to_string(log_level::trace) },
        { static_cast<int>(log_level::warn), log_level_to_string(log_level::warn) },
        { static_cast<int>(log_level::error), log_level_to_string(log_level::error) }
    };
...
log_level
log_registry::get_logger_level(std::string name) const {
    std::lock_guard<std::mutex> g(_mutex);
    return _loggers.at(name)->level();
}

void
log_registry::set_logger_level(std::string name, log_level level) {
    std::lock_guard<std::mutex> g(_mutex);
...
void log_registry::register_logger(logger* l) {
    std::lock_guard<std::mutex> g(_mutex);
    _loggers[l->name()] = l;
}

void log_registry::unregister_logger(logger* l) {
    std::lock_guard<std::mutex> g(_mutex);
    _loggers.erase(l->name());
}

void log_registry::moved(logger* from, logger* to) {
    std::lock_guard<std::mutex> g(_mutex);
    _loggers[from->name()] = to;
}

std::string pretty_type_name(const std::type_info& ti) {
    int status;
